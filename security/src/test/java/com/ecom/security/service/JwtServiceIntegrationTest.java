...
class JwtServiceIntegrationTest {
    ...
    void setUp() {
        ...
        when(devicesRepository.findByDeviceId("device-123")).thenReturn(Optional.of(existingDevice));
        when(devicesRepository.findByDeviceId("new-device")).thenReturn(Optional.empty());
    ...
    // Cas 2 : utilisateur actif avec un New device
    @Test
